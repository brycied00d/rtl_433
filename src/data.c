...
static void append_buf(data_printer_context_t *printer_ctx, const char *str)
{
	...
	if (syslog->size >= len + 1) {
		strcpy(syslog->buf, str);
		syslog->buf += len;
		syslog->size -= len;
	}
}
...
static void print_syslog_data_buf(data_printer_context_t *printer_ctx, data_t *data, char *format, FILE *file)
{
	_Bool separator = false;
	append_buf(printer_ctx, "{");
	while (data) {
...
static void print_syslog_data(data_printer_context_t *printer_ctx, data_t *data, char *format, FILE *file)
{
	...
	if (syslog->buf) {
		print_syslog_data_buf(printer_ctx, data, format, file);
		return;
	}
	...
	snprintf_a(&syslog->buf, &syslog->size, "<%d>1 %s %s rtl_433 - - - ", syslog->pri, timestamp, syslog->hostname);
...
{
	data_syslog_aux_t *syslog = printer_ctx->aux;
	snprintf_a(&syslog->buf, &syslog->size, "%f", data);
}

static void print_syslog_int(data_printer_context_t *printer_ctx, int data, char *format, FILE *file)
{
	data_syslog_aux_t *syslog = printer_ctx->aux;
	snprintf_a(&syslog->buf, &syslog->size, "%d", data);
}
